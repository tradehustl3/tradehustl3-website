...
path = Path("worker/resume-builder.ts")
text = path.read_text()

old = 'import { hardenGeneratedResumePackage } from "./resume-package-hardener";\n'
new = old + 'import { assessResumeExtractionCoverage, assessSavedIntakeExtractionCoverage } from "./resume-extraction-coverage";\n'
if "assessResumeExtractionCoverage" not in text:
    assert old in text
    text = text.replace(old, new, 1)

marker = 'function firstResumeEmail(text: string): string {'
if "IMPORT_RECONCILIATION_INSTRUCTION" not in text:
    insert = '''const IMPORT_RECONCILIATION_INSTRUCTION = `Extraction coverage reconciliation rule:
...
marker = 'function isResumeTheme(theme: unknown): theme is ResumeTheme {'
if "reconcileImportedExtraction" not in text:
    insert = '''async function reconcileImportedExtraction(
...
marker = 'async function enforceCheckoutQualityGate('
if "enforceUploadExtractionCoverageBeforeGeneration" not in text:
    insert = '''async function enforceUploadExtractionCoverageBeforeGeneration(
  request: Request,
  env: ResumeBuilderEnv,
  dependencies: ResumeBuilderDependencies,
  resumeId: string,
...
old = '  const importCopy = importPath ? request.clone() : null;\n'
new = (
...
marker = '  if (request.method === "POST" && checkoutPathMatch) {\n'
if 'enforceUploadExtractionCoverageBeforeGeneration(request' not in text:
    insert = (
...
old = '''  if (importPath && importCopy) {
    return preserveImportedContact(importCopy, first);
  }
'''
if old in text:
    new = '''  if (importPath && importContactCopy && importCoverageCopy && importRetryCopy) {
